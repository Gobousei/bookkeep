      ...
      if (user) {
        ...
        console.log(uid);
        db.ref('user/'+auth.currentUser.uid).once('value',function (obj){
          if(!obj.val()){
        ...
        db.ref('user/'+auth.currentUser.uid+'/balance').on('value',function (obj){
          balance = obj.val();
  ...
  function index(){
      ...
      db.ref('user/'+auth.currentUser.uid+'/month/'+month+'/data').once('value',function (obj){
        data = obj.val();
  ...
  function formsubmit(){
    ...
    db.ref('user/'+auth.currentUser.uid+'/month/'+month).update({data});

    })
    var balance;
    db.ref('user/'+auth.currentUser.uid+'/balance').once('value',function (obj){
    ...
    db.ref('user/'+auth.currentUser.uid).update({balance});
    setTimeout(function () {
      formclear();
  }, 1000);
